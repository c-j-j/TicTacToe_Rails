...
class TicTacToeController < ApplicationController
  ...
  def new_game
    ...
    redirect_to play_move_path(play_move_query_params(game.presenter.board_as_string))
  end
  ...
  def generate_display_fields(game)
    @next_turn_url =  play_move_path(play_move_query_params(game.presenter.board_as_string))
  ...
  def play_move_query_params(board_positions)
    {'game_type' => extract_game_type, 'board' => board_positions}
  end

  def build_game_from_params
    board = TicTacToe::GamePresenter.build_board_from_string(params['board'])
